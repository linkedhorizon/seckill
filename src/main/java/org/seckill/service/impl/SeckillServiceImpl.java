...
public class SeckillServiceImpl implements SeckillService{
	...
	//MD5盐值字符串，用于混淆MD5
	private final String salt="dfajw^^398TT3QU#`#dx&4543#2~@+782~cV";
	@Override
	 ...
	 * 2、保证事务执行方法的时间尽可能短，不要穿插其他网络操作，RPC/HTTP请求或者剥离到事务方法的外面
	 * 3、不是所有的方法都需要事务，如只有一条写入操作，只读操作不需要事务控制
	 */
		...
		if(insertCount <= 0){
			...
			try{
				...
				if(updateCount<=0){
					...
					return new SeckillExcution(seckillId, SeckillStateEnum.SUCESS, successKilled);
				}
	...
	public SeckillExcution excuteSeckillProcedure(long seckillId, long userPhone, String md5){
		...
		try {
			...
			if(result == 1){
				...
				return new SeckillExcution(seckillId,SeckillStateEnum.SUCESS,sk);
			}else{
